...
#         print("%s" %(json.dumps(result, indent=4)))
    ...
    def getValues(self, rangeName, valueRenderOption=None):
        self.checkQuota()
        try:
#             result = self.service.spreadsheets().values().get(spreadsheetId=self.spreadsheetId, range=rangeName).execute()
            result = self.service.spreadsheets().values().get(spreadsheetId=self.spreadsheetId, range=rangeName, valueRenderOption=valueRenderOption).execute()
        except Exception as e:
            print("can not read google sheet values: " + str(e))
            print("sleep 180 seconds and retry again")
#             traceback.print_exc()
            time.sleep(300)
            result = self.service.spreadsheets().values().get(spreadsheetId=self.spreadsheetId, range=rangeName).execute()
